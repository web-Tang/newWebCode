  ...
  methods: {
    ...
    commentsubmit(content) {
      ...
      let item = commentItem(
        ...
        content,
        createTime(nowtime)
      ); //comment(avatar, username, content, time)
    ...
    like() {
      ...
      if (!this.infolike) {
        ...
        if (this.$store.getters.getCurrentUser.likes == "") {
          idstr =
            this.$store.getters.getCurrentUser.likes +
    ...
    good() {
      ...
      if (!this.infogood) {
        ...
        if (this.$store.getters.getCurrentUser.goods == "") {
          idstr =
            this.$store.getters.getCurrentUser.goods +
            `${this.$route.params.id}`;
        } else {
          idstr =
            this.$store.getters.getCurrentUser.goods +
      ...
      highlight: function (str, lang) {
        ...
        if (lang && hljs.getLanguage(lang)) {
          try {
            ...
            let html = lines
              .map((item, index) => {
                ...
                if (index < 9) {
                  return `<li> ${
                    index + 1
                  }| <span class="line-num" data-line="${
                    index + 1
                  }"></span>${item}</li>`;
                } else if (9 <= index < 99) {
                  return `<li>${index + 1}| <span class="line-num" data-line="${
                    index + 1
                  }"></span>${item}</li>`;
                } else {
                  return `<li>${index + 1}| <span class="line-num" data-line="${
            ...
            return '<pre class="hljs myhljs"><code>' + html + "</code></pre>";
          } catch (__) {}
        }
        // 未添加代码语言，此处与上面同理
